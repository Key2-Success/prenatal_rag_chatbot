/**
 * chat/page.tsx — the app itself (moved off `/` so `/` can be a landing page).
 *
...
export default function ChatPage() {
  ...
  // Wake the backend on arrival so Render's ~1 min cold start overlaps the time
  // spent on the profile form. (The landing page pings it too, so by here it's
  // usually already awake.) Fires once; costs nothing (/health).
  useEffect(() => {
    warmUpBackend();
  }, []);
